...
var uiFontSize = parseInt(params['Font Size']);
var gMinDepo = parseInt(params['Minimum Depo']);
var intDefault = parseInt(params['Default Interest']);
...
Game_Banks.prototype.addBank = function(bID, bBalance, bInterest, bLastAccess, bTimeUnit,bIsBondSysActive, depoMin){
    if (!this._banks[bID]){
        this._banks[bID] = {};
        ...
        this._banks[bID]["minDeposit"] = depoMin;
    }
};
...
Game_Banks.prototype.setMinDepo = function(bID, depoMin){
	this._banks[bID]["minDeposit"] = depoMin;
};
...
Game_Banks.prototype.getMinDepo = function(bID){
	return this._banks[bID]["minDeposit"];
};
...
*/
var Geowil_Bank_DataManager_CreateGameObjects = DataManager.createGameObjects;
DataManager.createGameObjects = function(){
	Geowil_Bank_DataManager_CreateGameObjects.call(this,arguments);
	$gameBanks         = new Game_Banks();
};

var Geowil_Bank_DataManager_MakeSaveContents = DataManager.makeSaveContents;
DataManager.makeSaveContents = function() {
    // A save data does not contain $gameTemp, $gameMessage, and $gameTroop.
    var contents = {};
    contents = Geowil_Bank_DataManager_MakeSaveContents.call(this,arguments);
...
/* Image Manager */
ImageManager.loadBank = function(filename, hue) {
    return this.loadBitmap('img/bank/', filename, hue, false);
};


/* Game Interpreter */
var Geowil_Bank_Game_Interpreter_pluginCommand = Game_Interpreter.prototype.pluginCommand;
Game_Interpreter.prototype.pluginCommand = function(command, args){
	Geowil_Bank_Game_Interpreter_pluginCommand.call(this,command,args);
	var matches = [];
	var matches2 = [];
	var params = [];
	...
	if (command.match(/Bank[ ](\d+)[ ]AddBond[ ](\d+)[ ](\w.*.\w)[ ](\d+)[ ](\d+)[ ](\d+)[ ](\w.*.\w\W)/)){
		...
		while (match = re.exec(command)){
	  	...
	  	for (var i1 = 0; i1 < matches.length; i1++) {
	  		var param = [];
		    param = matches[i1].split(":");
		    
		    if (param[0] == "ID") { bID = parseInt(param[1]); }
		    else if (param[0] == "IR") { bInt = parseInt(param[1]); }
		    else if (param[0] == "TD") { tDom = param[1]; }
		    else if (param[0] == "MD") { mDepo = parseInt(param[1]); }
	  	}
		
		createBankRecord(bID,bInt,tDom,mDepo);
...
function createBankRecord(bankID,interestAmt, tDom, minDepo){
	if ($gameBanks.bankExists(bankID)){
		//Check if bank interest is 0 and if interest param is not
		if ($gameBanks.getInterest(bankID) !== interestAmt) { $gameBanks.setInterest(bankID,interestAmt); }
		if ($gameBanks.getTimeUnit(bankID) !== tDom) { $gameBanks.setTimeUnit(bankID,tDom); }
		if ($gameBanks.getMinDepo(bankID) !== minDepo) { $gameBanks.setMinDepo(bankID,minDepo); }

		bank = $gameBanks.getBank(bankID);			
		bankId = bankID;

		if ($gameBanks.getInterest(bankID) != 0) { calcInterest(); }
	} else{
		$gameBanks.addBank(bankID,0,interestAmt,Date.parse('0'),tDom,bIsBondSysActive,minDepo);
...
function calcInterest(){
	...
	if (timeDifference != 0){
		...
		bank["balance"] += Math.trunc((((tmDifference*bank["interest"])/100)*bank["balance"]));
	}
};
...
Window_ActionPane.prototype.initialize = function(x,y,w,h){
	...
	this._helpTxtList = ["Deposit " + TextManager.currencyUnit + " into your account","Withdraw " + TextManager.currencyUnit + " from your account"];
...
Window_ActionPane.prototype.drawActionItem = function(index, itmX, itmY, itmW){
	this.contents.fontSize = uiFontSize;
...
Window_Deposit.prototype.createButtons = function() {
    var bitmap = ImageManager.loadBank('ButtonSet');
    var buttonWidth = 48;
    var buttonHeight = 48;
    ...
    for (var i = 0; i < 6; i++) {
        ...
        var x = buttonWidth * (i < 5 ? i : i+1);
        var w = buttonWidth * (i < 4 ? 1 : 2);
...
Window_Deposit.prototype.onButtonOk = function() {
    if ($gameParty.gold() < this._newDepo || (this._bnk["minDeposit"] != 0 && this._newDepo < this._bnk["minDeposit"])) {
...
Window_Deposit.prototype.onButtonCancel = function(){
	this.processCancel();
};
...
Window_Withdraw.prototype.drawGold = function(bIsInvalid) {
    ...
    this.drawText(this._newWithdraw+this._currencyUnit, x, y, width, 'right');
};
...
Window_Withdraw.prototype.onButtonCancel = function(){
	this.processCancel();
};
...
Window_BankDetails.prototype.refresh = function() {
	...
	var yInc = 1;
	this.resetTextColor();
	this.drawText("Current " + TextManager.currencyUnit + ": " + $gameParty.gold(),x,y,210,36*1,'left');	

	if (this._bnk["interest"] > 0){
		y = 38*yInc;
		this.drawText("Interest Rate: " + this._bnk["interest"],x,y,210,36,'left');
		yInc++
	}
	
	y = 38*yInc;
	this.drawText("Balance: " + this._bnk["balance"],x,y,210,36*1,'left');
	yInc++;	

	if (Object.keys(this._bnk["bonds"]).length > 0){
		y = 38*yInc;
		this.drawText("Bonds: " + Object.keys(this._bnk["bonds"]).length,x,y,210,36*1,'left');
	}	
